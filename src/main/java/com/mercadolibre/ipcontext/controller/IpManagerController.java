...
public class IpManagerController {
    ...
    @GetMapping("/info/{ip}")
    public ResponseEntity<IpContextResponseDto> getIpInfo(
            @PathVariable(value = "ip") @IpAddress(message = "It is not a valid ip address") String ipAddress) {
        return new ResponseEntity<>(ipContextService.getIpContext(ipAddress), HttpStatus.OK);
    }

    @PostMapping("/banIp")
    public ResponseEntity<GetIpBlacklistDto> addIpAddressToBlacklist(
            @Valid @RequestBody AddIpBlacklistDto addIpBlacklistDto) {
